...
| P7 | `docs/superpowers/plans/2026-06-22-live-scoring-loop.md` | Live scoring loop | 🟡 | **L0 ✅ L1 ✅** L0 `score_ledger`; L1 `run_daily_update` (as-of-aware, deterministic `generated_at_utc`, idempotent + prior-partition-immutable, graceful no-fixture skip). Made `generated_at_utc` injectable in forecast_live; made the lab pyarrow-free (duckdb reads in run_championship_odds/elo_vs_market/recency_experiment; openpyxl declared). Full suite 113 green. Next: L2 scorecard (closes roadmap). |
